...
public class MetreReport {
    ...
    public void printTotalEmpReport(String datee, String UsName) throws DocumentException, FileNotFoundException, SQLException {
        ...
        String query = "SELECT Sum(o.orderCount) , o.orderProduct FROM `order` o  where o.orderProduct = o.orderProduct And orderDate LIKE '%" + datee + "%' AND userName Like'%" + UsName + "%' And orderKind='سفرة' group by o.orderProduct";
        ...
        document.add(new Paragraph(" ", f));
        table.flushContent();
        IC.pst = IC.dbc.conn.prepareStatement("SELECT Sum(o.orderTotal) FROM `order` o  where orderDate LIKE '%" + datee + "%' AND userName Like'%" + UsName + "%' And orderKind='سفرة'");
        IC.rs = IC.pst.executeQuery();
        if (IC.rs.next()) {
            ///orderNum
        ...
        document.add(new Paragraph(" ", f));
        table.flushContent();
        IC.pst = IC.dbc.conn.prepareStatement("SELECT COUNT(DISTINCT orderNum) FROM `order`  where orderDate LIKE '%" + datee + "%' AND userName Like'%" + UsName + "%' And orderKind='سفرة'");
        IC.rs = IC.pst.executeQuery();
        if (IC.rs.next()) {
            ///orderNum
    ...
    public void printEmpReport(String datee, String UsName) throws DocumentException, FileNotFoundException, SQLException {
        ...
        String query = "SELECT * FROM `order` WHERE orderDate LIKE '%" + datee + "%' AND userName Like'%" + UsName + "%' And orderKind='سفرة'";
        IC.pst = IC.dbc.conn.prepareStatement(query);
        IC.rs = IC.pst.executeQuery();
        while (IC.rs.next()) {
            ///orderNum
